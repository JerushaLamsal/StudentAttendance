...
if(isset($_POST["action"]))
{
    ...
    {
        $query =" SELECT * FROM tbl_student INNER JOIN tbl_grade ON tbl_grade.grade_id = tbl_student. student_grade_id";
        ...
        if($_POST["action"]== 'Add' || $_POSTa["action"] == "Edit")
        {
            $student_name= '';
            $student_roll_number= '';
            $student_dob= '';
            ...
            {
                $error_student_grade_id = 'Student Grade Id is required';
                $error++;
            }
            else{
...
if($error>0)
{
    $output= array(
        ...
        'error_studemt_roll_number'   =>$error_student_roll_number,
...
if($statement = $execute($data))
{
   ...
   if($_POST["action"] == "Edit")
   {
    ...
    $query = " 
    UPDATE tbl_student SET student_name =:student_name,
    student_roll_number =:student_roll_number,
    student_dob =:student_dob,
    student_grade_id =:student_grade_id WHERE
...
echo json_encode($output);
        ...
        {
            $query = "
            SELECT * FROM tbl_student WHERE student_id = '". $_POST["student_id"]."'
            ";
            $statement = $connect->prepare($query);
                ...
                foreach($result as $row)
{
  ...
  echo json_encode($output);
        ...
        {
            $query = "DELETE FROM tbl_student WHERE student_id = '".$_POST["student_id"]."'
            ";
            $statement = $connect->prepare($query);
            if($statement->execute())
            {
