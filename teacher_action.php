...
if(isset($_POST["action"]))
{
    ...
    {
        $query = " SELECT * FROM tbl_teacher INNER JOIN tbl_grade ON tbl_grade.grade_id = tbl_teacher. teacher_grade_id "; 
        ...
        if($_POST["length"] != -1)
        {
          ...
          {
            $sub_array = array();
            $sub_array[] = '<img src="https://localhost/StudentAttendance/teacher_image/' .$row["teacher_image"]. '"
    ...
    if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
    {
      ...
      {
        $file_name = $_POST["teacher_image"]["name"];
        $tmp_name = $FILES["teacher_image"]["tmp_name"];
...
if($_POST["action"] == "Add")
{
  if(empty($_POST["teacher_emailid"]))
  {
    ...
    if(!filter_val($_POST["teacher_emailid"], FILTER_VALIDATE_EMAIL))
    {
...
$query ="
INSERT INTO tbl_teacher(teacher_name, teacher_address, teacher_emailid,teacher_password,teacher_qualification,teacher_doj,teacher_image,teacher_grade_id)
SELECT * FROM (SELECT :teacher_name, :teacher_address,:teacher_emailid,:teacher_password,:teacher_qualification,
:teacher_doj,
:teacher_image,
:teacher_grade_id) as temp WHERE NOT EXISTS ( SELECT teacher_emailid FROM tbl_teacher WHERE teacher_emailid = :teacher_emailid) LIMIT 1";
   $statement =$connect->prepare($query);
...
$output = array(
  'error'    => true,
  'error_teacher_emaildid'    =>'Email Already Exists'
);
    }
   }
  }
...
{
  $query =" SELECT * FROM tbl_teacher  INNER JOIN tbl_grade ON tbl_grade.grade_id = tbl_teacher. teacher_grade_id WHERE tbl_teacher.teacher_id = '".$_POST["teacher_id"]."'
  ";
$statement= $connect->prepare($query);
